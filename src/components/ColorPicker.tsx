    ...
    <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-12 gap-12 items-center fade-in">
      ...
      <div className="md:col-span-7 flex flex-col items-center md:items-start space-y-6">
        <div className="relative group">
          ...
          <div className="absolute -bottom-4 -right-4 bg-surface-container-highest px-4 py-2 rounded-lg backdrop-blur-xl border border-outline-variant/10 shadow-xl">
            <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-primary">Your Match</span>
          </div>
        </div>
      ...
      <div className="md:col-span-5 flex flex-col space-y-12 bg-surface-container-low p-8 rounded-xl relative shadow-2xl">
        ...
        <div className="space-y-10">
          ...
          <Slider label="Saturation" value={saturation} min={0} max={100} onChange={setSaturation} />
          <Slider label="Brightness" value={brightness} min={0} max={100} onChange={setBrightness} />
        </div>
        ...
        <div className="pt-4">
          ...
          >
            ...
            <span className="relative text-on-surface group-hover:text-on-primary-container z-10 transition-colors">Submit Match</span>
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <label className="text-[10px] uppercase tracking-widest font-bold text-on-surface-variant">{label}</label>
        <span className="text-[10px] font-mono text-on-surface-variant opacity-40">{Math.round(value)}</span>
      </div>
